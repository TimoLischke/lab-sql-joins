...
-- AS "short letter code" can be introduced to shorten your code
SELECT COUNT(film_id) AS nr_films, c.name
FROM film AS f INNER JOIN film_category AS fc USING (film_id)
INNER JOIN category AS c USING (category_id) GROUP BY c.name;
...
FROM film AS f LEFT JOIN inventory AS i USING (film_id)
GROUP BY f.title, availability ORDER BY availability DESC;